...
export function BranchForm() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { code: "", name: "", address: "", phone: "", isActive: true },
  });
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (!res.ok) {
      ...
      setServerError(data?.error ?? "Failed to create branch");
      return;
    }
    ...
    <div className="grid gap-4">
      ...
      <form className="grid max-w-2xl gap-4" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link className="rounded-xl border px-4 py-2 text-sm hover:bg-zinc-50" href="/app/settings/branches">
            Cancel / إلغاء
          </Link>
        </div>
      </form>
