...
export function EstimateForm({ customers: initialCustomers, products, costCenters, branches, baseCurrencyCode, defaultCustomerId, defaultBranchId }: Props) {
  ...
  useEffect(() => {
    fetch("/api/estimates")
      .then((r) => r.json())
      .then((d: { nextNumber?: string }) => { if (d.nextNumber) form.setValue("estimateNumber", d.nextNumber); })
      .catch(() => {});
  ...
  const handleAddCustomer = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/customers", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: newCustName.trim(), phone: newCustPhone.trim() || undefined, email: newCustEmail.trim() || undefined, companyName: newCustCompanyName.trim() || undefined }) });
      ...
      if (res.ok && data.id) {
        ...
        setShowNewCustomer(false); setNewCustName(""); setNewCustPhone(""); setNewCustEmail(""); setNewCustCompanyName("");
      } else { setServerError(data.error || "Failed to create customer"); }
    } catch { setServerError("Failed to create customer"); }
  ...
  async function submit(values: SubmitValues, mode: "DRAFT" | "SEND") {
    setServerError(null);
    const payload = {
      ...values, mode, branchId: values.branchId ?? "",
    ...
    if (!res.ok) { const parsedErr = ApiErrSchema.safeParse(data); setServerError(parsedErr.success ? parsedErr.data.error : `Failed to create estimate (HTTP ${res.status})`); return; }
    const parsedOk = ApiOkSchema.safeParse(data);
    if (!parsedOk.success) { setServerError("Created but no id returned"); return; }
    router.push(`/app/estimates/${parsedOk.data.id}`);
    router.refresh();
  }
      ...
      <form className="grid gap-5">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <input className="mt-1 w-full rounded-xl border px-3 py-2 font-mono text-sm" placeholder="EST-0001" {...form.register("estimateNumber")} />
            {errors.estimateNumber && <div className="mt-1 text-xs text-red-600">Estimate number is required.</div>}
          </div>
          ...
          <div>
            ...
            <div className="flex items-start gap-2">
              <div className="flex-1 min-w-0">
                ...
                <CustomerAutocompleteField customers={customers} defaultCustomerId={initialCustomerId} placeholder="Search customer / ابحث عن زبون" noResultsLabel="No customers found" clearLabel="Clear" disabled={customers.length === 0} onSelectedIdChange={(id) => form.setValue("customerId", id, { shouldDirty: true, shouldValidate: true })} />
              </div>
              <button type="button" className="mt-1 shrink-0 rounded-xl border px-3 py-2 text-sm hover:bg-zinc-50" title="Add new customer" onClick={() => setShowNewCustomer(true)}>+</button>
            </div>
          ...
          <div>
            ...
            <select className="mt-1 w-full rounded-xl border px-3 py-2 text-sm" {...form.register("branchId")} disabled={branches.length === 0}>
              ...
              {branches.map((b) => <option key={b.id} value={b.id}>{b.code} — {b.name}{b.isActive === false ? " (Inactive)" : ""}</option>)}
            </select>
          </div>

          <div>
            <label className="text-sm font-medium text-zinc-700">Company name / اسم الشركة</label>
            <input className="mt-1 w-full rounded-xl border bg-zinc-50 px-3 py-2 text-sm text-zinc-700" value={selectedCustomer?.companyName ?? ""} readOnly placeholder="Will appear automatically" />
          </div>
          ...
          <div>
            <label className="text-sm font-medium text-zinc-700">Expiry date / تاريخ الانتهاء</label>
            <input className="mt-1 w-full rounded-xl border px-3 py-2 text-sm" type="date" {...form.register("expiryDate")} />
          </div>
          ...
          {showFx && (
            <div className="md:col-span-2">
              ...
              <div className="mt-1 flex items-center gap-2">
                <span className="text-sm text-zinc-600">1 {currencyCode} =</span>
                <input className="w-40 rounded-xl border px-3 py-2 text-sm" placeholder="e.g. 1300" {...form.register("exchangeRate")} />
                <span className="text-sm text-zinc-600">{baseCurrencyCode}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div>
            <label className="text-sm font-medium text-zinc-700">Note / ملاحظات</label>
            <input className="mt-1 w-full rounded-xl border px-3 py-2 text-sm" {...form.register("note")} placeholder="Optional note..." />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4">
          <div className="text-sm font-medium text-sky-900 mb-3">Estimate Summary / ملخص عرض السعر</div>
          <div className="grid gap-2 text-sm">
            <div className="flex justify-between"><span className="text-zinc-600">Subtotal / المجموع الفرعي</span><span className="font-mono font-medium">{fmtNum(totals.subtotal)} {currencyCode}</span></div>
            {totals.discountAmount > 0 && <div className="flex justify-between text-amber-700"><span>Discount / خصم {watchedDiscountType === "PERCENTAGE" ? `(${watchedDiscountValue}%)` : ""}</span><span className="font-mono font-medium">-{fmtNum(totals.discountAmount)} {currencyCode}</span></div>}
            {totals.taxTotal > 0 && <div className="flex justify-between"><span className="text-zinc-600">Tax / ضريبة</span><span className="font-mono font-medium">{fmtNum(totals.taxTotal)} {currencyCode}</span></div>}
            <div className="flex justify-between border-t border-sky-200 pt-2 text-base font-bold"><span className="text-sky-900">Net Total / الصافي</span><span className="font-mono text-sky-700">{fmtNum(totals.total)} {currencyCode}</span></div>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <button className="rounded-xl border px-4 py-2 text-sm hover:bg-zinc-50" type="button" onClick={form.handleSubmit((v) => submit(v, "DRAFT"), () => setServerError("Please complete the required fields."))} disabled={customers.length === 0 || isSubmitting}>
            {isSubmitting ? "Saving..." : "Save draft / حفظ مسودة"}
          </button>
          <button className="rounded-xl bg-zinc-900 px-4 py-2 text-sm text-white hover:bg-zinc-800" type="button" onClick={form.handleSubmit((v) => submit(v, "SEND"), () => setServerError("Please complete the required fields."))} disabled={customers.length === 0 || isSubmitting}>
            {isSubmitting ? "Sending..." : "Send / إرسال"}
          </button>
        </div>
      </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-md rounded-2xl border bg-white p-6 shadow-lg">
            ...
            <div className="grid gap-3">
              <div><label className="text-sm font-medium text-zinc-700">Name / الاسم *</label><input className="mt-1 w-full rounded-xl border px-3 py-2" value={newCustName} onChange={(e) => setNewCustName(e.target.value)} placeholder="Customer name" autoFocus /></div>
              <div><label className="text-sm font-medium text-zinc-700">Company Name / اسم الشركة</label><input className="mt-1 w-full rounded-xl border px-3 py-2" value={newCustCompanyName} onChange={(e) => setNewCustCompanyName(e.target.value)} /></div>
              <div><label className="text-sm font-medium text-zinc-700">Email</label><input className="mt-1 w-full rounded-xl border px-3 py-2" type="email" value={newCustEmail} onChange={(e) => setNewCustEmail(e.target.value)} /></div>
              <div><label className="text-sm font-medium text-zinc-700">Phone</label><input className="mt-1 w-full rounded-xl border px-3 py-2" value={newCustPhone} onChange={(e) => setNewCustPhone(e.target.value)} /></div>
            </div>
            <div className="mt-4 flex items-center gap-3">
              <button type="button" className="rounded-xl bg-zinc-900 px-4 py-2 text-sm text-white hover:bg-zinc-800" onClick={handleAddCustomer} disabled={newCustSaving || !newCustName.trim()}>{newCustSaving ? "Saving..." : "Save / حفظ"}</button>
              <button type="button" className="rounded-xl border px-4 py-2 text-sm hover:bg-zinc-50" onClick={() => setShowNewCustomer(false)}>Cancel / إلغاء</button>
            </div>
          </div>
        </div>
      )}
