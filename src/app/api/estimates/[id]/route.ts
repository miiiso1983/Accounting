...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const estimate = await prisma.estimate.findFirst({
    ...
    include: { customer: true, exchangeRate: true, lineItems: true, convertedInvoice: { select: { id: true, invoiceNumber: true } } },
  });
...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      const existing = await tx.estimate.findFirst({ where: { id, companyId: company.id }, select: { id: true, status: true } });
      if (!existing) throw new Error("Estimate not found");
      if (existing.status !== "DRAFT") throw new Error("Only DRAFT estimates can be edited");
      ...
      if (estimateCurrency !== baseCurrencyCode) {
        ...
        const fx = await tx.exchangeRate.create({
          data: { companyId: company.id, baseCurrencyCode: estimateCurrency, quoteCurrencyCode: baseCurrencyCode, rate: rateStr, effectiveAt: issueDate, source: "estimate-manual" },
          select: { id: true },
        });
      ...
      const computedLines = body.lines.map((l) => {
        ...
        if (lineDiscountType === "PERCENTAGE" && lineDiscountValue.gt(0)) lineDiscountAmount = grossTotal.mul(lineDiscountValue).div(100).toDecimalPlaces(6);
        else if (lineDiscountType === "FIXED" && lineDiscountValue.gt(0)) lineDiscountAmount = lineDiscountValue.toDecimalPlaces(6);
        ...
        return { description: l.description, costCenterId: l.costCenterId?.trim() || null, productId: l.productId?.trim() || null, quantity: qty, unitPrice: price, discountType: lineDiscountType, discountValue: lineDiscountValue, lineTotal, taxRate, lineTax };
      });
      ...
      if (discountType === "PERCENTAGE" && discountValue.gt(0)) discountAmount = subtotal.mul(discountValue).div(100).toDecimalPlaces(6);
      else if (discountType === "FIXED" && discountValue.gt(0)) discountAmount = discountValue.toDecimalPlaces(6);
      ...
      const est = await tx.estimate.update({
        ...
        data: {
          ...
          subtotalBase: mulToBase(subtotal).toDecimalPlaces(6).toFixed(6),
          discountAmountBase: mulToBase(discountAmount).toDecimalPlaces(6).toFixed(6),
          taxTotalBase: mulToBase(taxTotal).toDecimalPlaces(6).toFixed(6),
          totalBase: mulToBase(total).toDecimalPlaces(6).toFixed(6),
          note: body.note?.trim() || null,
          lineItems: {
            create: computedLines.map((l) => ({
              description: l.description, costCenterId: l.costCenterId, productId: l.productId,
              quantity: l.quantity.toDecimalPlaces(6).toFixed(6), unitPrice: l.unitPrice.toDecimalPlaces(6).toFixed(6),
              discountType: l.discountType, discountValue: l.discountValue.toDecimalPlaces(6).toFixed(6),
              lineTotal: l.lineTotal.toDecimalPlaces(6).toFixed(6), taxRate: l.taxRate ? l.taxRate.toDecimalPlaces(6).toFixed(6) : null,
            })),
          },
        },
        select: { id: true },
