  ...
  return (
    <div className="rounded-2xl border p-4">
      <div className="flex items-center justify-between gap-4 mb-3">
        ...
        <button type="button" className="rounded-xl border px-3 py-2 text-sm hover:bg-zinc-50" onClick={onAppend}>Add line / إضافة بند</button>
      </div>
        ...
        <table ref={tableRef} className="w-full border-collapse" style={{ tableLayout: "fixed" }}>
          ...
          <thead>
            <tr className="border-b border-zinc-200">
              {labels.map((l, i) => (
                <th key={i} className="relative px-1 py-2 text-xs font-medium text-zinc-500 text-right select-none whitespace-nowrap">
                  {l}
                  {i < labels.length - 1 && (
                    <div className="absolute top-0 right-0 h-full w-1.5 cursor-col-resize hover:bg-sky-300 active:bg-sky-400 z-10"
          ...
          <tbody>
            {fields.map((f, idx) => {
              ...
              return (
                <tr key={f.id} className="border-b border-zinc-50 hover:bg-zinc-50/50 group">
                  {hasProducts && <GridProductCell idx={idx} products={products} costCenters={costCenters} setValue={setValue} />}
                  <td className="px-1 py-1"><input className="w-full border-0 bg-transparent px-1 py-1.5 text-sm focus:ring-1 focus:ring-sky-300 rounded" placeholder="الوصف" {...register(`lines.${idx}.description`)} onFocus={autoSelect} onInput={e => handleInput(pIdx, (e.target as HTMLInputElement).value)} />{errs?.lines?.[idx]?.description && <div className="text-[10px] text-red-600">مطلوب</div>}</td>
                  <td className="px-1 py-1"><select className="w-full border-0 bg-transparent px-1 py-1.5 text-sm focus:ring-1 focus:ring-sky-300 rounded" {...register(`lines.${idx}.costCenterId`)}><option value="">—</option>{costCenters.map(cc => <option key={cc.id} value={cc.id}>{cc.code} — {cc.name}</option>)}</select></td>
                  <td className="px-1 py-1"><input className="w-full border-0 bg-transparent px-1 py-1.5 font-mono text-sm focus:ring-1 focus:ring-sky-300 rounded" inputMode="decimal" placeholder="1" {...register(`lines.${idx}.quantity`)} onFocus={autoSelect} onInput={e => handleInput(pIdx + 2, (e.target as HTMLInputElement).value)} /></td>
                  <td className="px-1 py-1"><input className="w-full border-0 bg-transparent px-1 py-1.5 font-mono text-sm focus:ring-1 focus:ring-sky-300 rounded" inputMode="decimal" placeholder="0" {...register(`lines.${idx}.unitPrice`)} onFocus={autoSelect} onInput={e => handleInput(pIdx + 3, (e.target as HTMLInputElement).value)} /></td>
                  <td className="px-1 py-1"><select className="w-full border-0 bg-transparent px-1 py-1.5 text-xs focus:ring-1 focus:ring-sky-300 rounded" {...register(`lines.${idx}.discountType`)}><option value="">—</option><option value="PERCENTAGE">%</option><option value="FIXED">ثابت</option></select></td>
                  <td className="px-1 py-1"><input className="w-full border-0 bg-transparent px-1 py-1.5 font-mono text-sm focus:ring-1 focus:ring-sky-300 rounded" inputMode="decimal" placeholder="0" {...register(`lines.${idx}.discountValue`)} onFocus={autoSelect} onInput={e => handleInput(pIdx + 5, (e.target as HTMLInputElement).value)} /></td>
                  <td className="px-1 py-1"><input className="w-full border-0 bg-transparent px-1 py-1.5 font-mono text-sm focus:ring-1 focus:ring-sky-300 rounded" inputMode="decimal" placeholder="0" {...register(`lines.${idx}.taxRate`)} onFocus={autoSelect} onInput={e => handleInput(pIdx + 6, (e.target as HTMLInputElement).value)} /></td>
                  <td className="px-1 py-1 text-right"><span className="font-mono text-sm font-medium text-zinc-900">{fmtNum(lineTotals[idx] ?? 0)}</span></td>
                  <td className="px-1 py-1 text-center"><button type="button" className="text-sm text-zinc-400 hover:text-red-500 disabled:opacity-30" onClick={() => onRemove(idx)} disabled={!canRemove} title="حذف">×</button></td>
                </tr>
              );
            })}
          </tbody>
